...
export default function AnalyzePage() {
  ...
  function getConfidenceColor() {
    if (analysis?.confidence === "high") return "bg-brand/10 text-brand";
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center gap-3 px-5 py-2.5 border-b border-white/[0.06] bg-neutral-950 shrink-0">
        <div className="flex items-center gap-1.5 bg-white/[0.04] border border-white/[0.06] rounded-md px-3 py-1.5 text-xs font-semibold text-white">
            ...
            <button
              ...
              className={`px-2.5 py-1 text-[11px] font-medium rounded transition ${
                tf === activeTf
                  ...
                  : "bg-white/[0.04] border border-white/[0.06] text-neutral-600 hover:text-neutral-500"
              }`}
            >
              {tf}
            </button>
        ...
        <label className="flex items-center gap-1.5 bg-white/[0.04] border border-white/[0.06] rounded-md px-3 py-1.5 text-xs text-neutral-500 cursor-pointer hover:border-brand/30 hover:text-brand transition">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 bg-brand text-black px-4 py-1.5 rounded-md text-xs font-bold hover:bg-brand-light transition disabled:opacity-40"
        >
          <Zap className="w-3.5 h-3.5" />
          {loading ? "Analyzing..." : "Analyze"}
        </button>
        {remaining !== null && typeof remaining === "number" && (
          <span className="text-[11px] text-neutral-600">{remaining} left today</span>
        )}
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex items-center justify-center p-8 overflow-auto">
          {image ? (
            ...
            <img src={image} alt="Chart" className="max-w-full max-h-full rounded-lg border border-white/[0.06] object-contain" />
          ) : (
            <div className="text-center">
              ...
              <label className="inline-flex items-center gap-2 bg-brand text-black px-6 py-3 rounded-xl text-sm font-bold cursor-pointer hover:bg-brand-light transition">
                <Upload className="w-4 h-4" />
                Upload Chart Screenshot
                <input type="file" accept="image/*" className="hidden" onChange={handleUpload} />
              </label>
              <p className="text-xs text-neutral-700 mt-3">Supports PNG, JPG, WebP</p>
            </div>
          )}
        </div>

        {/* AI Panel */}
        <div className="w-[360px] border-l border-white/[0.06] bg-neutral-950 flex flex-col shrink-0">
              ...
              {(["analysis", "strategy", "learn"] as const).map((tab) => (
                <button
                  ...
                  className={`flex-1 py-2.5 text-[11px] font-semibold uppercase tracking-wider border-b-2 transition ${
                    activeTab === tab ? "text-brand border-brand" : "text-neutral-700 border-transparent hover:text-neutral-500"
                  }`}
                >
                  {tab}
                </button>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            {loading && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-neutral-500">Analyzing chart patterns...</p>
                <p className="text-xs text-neutral-700 mt-1">This takes 5-10 seconds</p>
              </div>
            )}
            ...
            {analysis && activeTab === "analysis" && (
              <div className="space-y-4">
                <div className="space-y-2">
                  <h3 className="text-[10px] uppercase tracking-wider text-neutral-700 font-semibold">Trade Setup</h3>
                ...
                <div className="space-y-2">
                  <h3 className="text-[10px] uppercase tracking-wider text-neutral-700 font-semibold">Entry & Exit</h3>
                ...
                <div className="space-y-2">
                  <h3 className="text-[10px] uppercase tracking-wider text-neutral-700 font-semibold">Risk</h3>
                  ...
                  <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-3">
                    <div className="text-[10px] text-neutral-600 mb-2">Signals</div>
                    <div className="flex flex-wrap gap-1">
                      {analysis.signals.map((s, i) => (
                        <span key={i} className="text-[10px] font-semibold bg-brand/10 text-brand px-2 py-0.5 rounded">
                          {s}
                        </span>
                      ))}
                    </div>
            ...
            {analysis && activeTab === "strategy" && (
              <div className="space-y-4">
                <h3 className="text-[10px] uppercase tracking-wider text-neutral-700 font-semibold">How to Execute</h3>
                <ol className="space-y-2">
                  {analysis.steps.map((step, i) => (
                    <li key={i} className="flex gap-3 text-xs text-neutral-300 leading-relaxed">
                      <span className="w-5 h-5 rounded-full bg-brand/10 text-brand text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                ...
                <div className="bg-amber-500/5 border border-amber-500/15 rounded-lg p-3 mt-4">
                  ...
                  <p className="text-xs text-neutral-300 leading-relaxed">{analysis.mistakes}</p>
                </div>
              </div>
            )}
            ...
            {analysis && activeTab === "learn" && (
              <div>
                <h3 className="text-[10px] uppercase tracking-wider text-neutral-700 font-semibold mb-3">Why This Trade?</h3>
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-4">
                  <p className="text-xs text-neutral-300 leading-[1.8] whitespace-pre-line">
...
function InfoCard({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg px-3 py-2.5 flex items-center justify-between">
